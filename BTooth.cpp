...
// need access to this objects methods from more than one function, removed from connectToServer(),
// see handleBluetooth()
...
bool connectToServer() {
    ...
    // moved to the top of the file to make it global, otherwise no access to the objekts methods after connect
    // BLEClient*  pClient  = BLEDevice::createClient();
...
void handleBluetooth(){
  ...
  // obtain information for diagnosis and display every <BT_STATUS_DELAY> milliseconds
  // needs access to pClient, thus pCient is created in global scope, see my changes above
  if (millis() - lastBTStatus > BT_STATUS_DELAY){

    BTRssiValue = pClient->getRssi();
    BTIsConnected = pClient->isConnected();
