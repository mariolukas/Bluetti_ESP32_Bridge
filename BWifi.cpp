...
#include <ESPAsyncWebSrv.h> // https://github.com/me-no-dev/ESPAsyncWebServer/archive/master.zip
...
unsigned long lastConnectStatus = 0;
...
// tried to avoid modem sleep as potential reason for disconnects, sometimes WiFi status is "idle"
// when this happens, there is no return to "connected" status by now -> ISSUE
...
void initBWifi(bool resetWifi){
  ...
  while (WiFi.status() != WL_CONNECTED) {
    if ((millis() - lastConnectStatus) > (WIFI_WAIT_CONNECT_DELAY * 1000)) {

      Serial.print(".");
      lastConnectStatus = millis();
...
// Attention! This seems to create significant workload for the processor
// after adding display support there are frequent error messages via serial console
// when the status website is displayed by a web browser.
// If I avoid using the ESPs website, all things seem to work properly (AlexBurghardt)
